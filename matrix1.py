...
class Box:
    ...
    for num in range(100):
        bitmaps.append(my_font.render(str(num), True, (64, 64, 64)))
    ...
    def draw(self):
        ...
        if self.number > 99:
            return
        if self.number < 0:
            pos_x, pos_y = self.number_rect.center
            pygame.draw.line(SURFACE, (64, 64, 64), (pos_x - 20, pos_y), (pos_x - 10, pos_y), 2)
        SURFACE.blit(self.bitmaps[abs(self.number)], self.number_rect)
        if self.is_focused:
            pygame.draw.rect(SURFACE, (0, 0, 225), self.focus_rect, 2)


def main():
    """main routine"""
    focus_index = 0
    positions = ((220, 520, 1), (280, 520, 0), (220, 560, 0), (280, 560, 1))
    boxes = []
    ...
    while True:
        ...
        for index in range(4):
            boxes[index].is_focused = True if index == focus_index else False
        ...
        dst = [(boxes[0].number * p[0] + boxes[1].number * p[1], boxes[2].number * p[0] + boxes[3].number * p[1]) for p
               in src]
        ...
        for index in range(0, 501, +25):
            pygame.draw.line(SURFACE, (64, 64, 64), (0, index), (500, index))
            pygame.draw.line(SURFACE, (64, 64, 64), (index, 0), (index, 500))
